...
// the surrounding #include directive.
    ...
    static void handle_attachment_get(AsyncWebServerRequest* req) {
      ...
      // Stream the file through the shared crash-safe chunked producer
      // (Web::FileStream::begin). The backing file lives in a shared_ptr
      // captured by both the reader and the destroy closure, so it stays
      // open for the life of the response and closes exactly once when
      // AsyncWebServer tears the response down. SD reads share the HSPI
      // bus with the IMU pump, so the SD reader guards each read.
      size_t total = 0;
      AsyncWebServerResponse* resp = nullptr;
      if (on_sd) {
        auto fp = std::make_shared<File>(Storage::SDCard::open_read(full.c_str()));
        if (!*fp) {
          NOTICEF("[ATTDBG] SD open_read failed for %s", full.c_str());
          send_error(req, 500, "attachment_open_failed");
          return;
        }
        total = (size_t)fp->size();
        resp = Web::FileStream::begin(req, total, "application/octet-stream",
        ...
        auto fp = std::make_shared<microStore::File>(
            filesystem.open(full.c_str(), microStore::File::ModeRead));
        if (!*fp) {
        ...
        total = (size_t)fp->size();
        resp = Web::FileStream::begin(req, total, "application/octet-stream",
            [fp](uint8_t* dst, size_t want) -> size_t { return (size_t)fp->read(dst, want); },
            [fp]() { if (*fp) fp->close(); });
      }
