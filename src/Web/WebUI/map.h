...
//
// Map tiles for the web app's location view. Tiles are vector .pmtiles
// layers on the SD card under /maps/layers (a world base plus any
// downloaded detail areas), each streamed on demand with HTTP range
// support through the shared crash-safe producer; the browser caches
// them. With no card or no layers the map falls back to coordinates
// only. Settings (source / zoom) persist via Web::MapConfig.

    // Highest zoom accepted for /api/map/config and the extract jobs.
    static constexpr int MAP_MAX_ZOOM = Web::MapConfig::MAX_ZOOM;

    // GET /api/map/config - the persisted settings plus whether an SD card is
    // present. The vector layers themselves are listed by /api/map/layers.
